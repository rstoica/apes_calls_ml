...
load apecalls.train -ascii;
callsTrain = apecalls;
load apecalls.test -ascii;
callsTest = apecalls;

% callsTrain and callsTest contain the 12-dim time series, which have
...
for c = 1:270
    ...
    l = 0;    
    while callsTrain(readindex, 1) ~= 1.0
        l = l+1;
        readindex = readindex + 1;
    end
    trainInputs{c,1} = callsTrain(readindex-l:readindex-1,:);    
end
...
for c = 1:370
    readindex = readindex + 1;
    l = 0;    
    while callsTest(readindex, 1) ~= 1.0
        l = l+1;
        readindex = readindex + 1;
    end
    testInputs{c,1} = callsTest(readindex-l:readindex-1,:);    
end
